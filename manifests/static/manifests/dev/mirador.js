...
//! OpenSeadragon 1.0.0
//! Built on 2014-03-18
//! Git commit: v1.0.0-41-ge73e5a9-dirty
    ...
    stitchView: {
      ...
      navToolbar: Handlebars.compile([
        '<div class="{{navToolbarCls}}">',
          ...
          '<a href="javascript:;" class="mirador-btn mirador-icon-metadata-view"></a>',
	//'<a href="javascript:;" class="mirador-btn mirador-icon-scroll-view"></a>',
	//collectionTileMargin: 60,'<a href="javascript:;" class="mirador-btn mirador-icon-thumbnails-view"></a>',
          '<a href="javascript:;" class="mirador-btn mirador-icon-previous"></a>',
          '<a href="javascript:;" class="mirador-btn mirador-icon-next"></a>',
        '</div>'
      ].join('')),
...
(function($){
  ...
  $.Manifest.prototype = {
	...
	'viewingHint':  this.jsonLd.viewingHint || 'paged',
	'viewingDirection': this.jsonLd.viewingDirection || 'left-to-right'
      };
...
(function($) {
  ...
  $.WidgetToolbar = function(options) {
    ...
    jQuery.extend(true, this, {
      ...
      id:             'mirador-widget-toolbar-' + $.genUUID()
    }, options);
...
(function($) {
  ...
  $.ImageView = function(options) {
    ...
    jQuery.extend(this, {
      ...
      showThumbnailNavigator: $.viewer.showThumbnailNavigator
    }, options);
  ...
  $.ImageView.prototype = {
	...
	'showNavigator': this.showThumbnailNavigator
      });
      ...
      if (this.hideOSDFullScreen) {
          //this.parent.toolbar.element.find('.' + this.osdToolbarCls + ' button:last-child').hide();
          this.parent.toolbar.element.find('button[title="Toggle full page"]').hide();
      }
...
(function($) {
  ...
  $.StitchView = function(options) {
    ...
    this.viewingDirection = this.metadata.details.viewingDirection;
    this.viewingHint = this.metadata.details.viewingHint;
    ...
    //console.log(this.stitchList);
  };


  $.StitchView.prototype = {
    ...
    createOpenSeadragonInstance: function(stitchList, osdBounds) {
      ...
      this.osd = $.OpenSeadragon({
        'id':           elemOsd.attr('id'),
        'toolbar':      this.parent.toolbar.id,//osdToolBarId,
	...
	'collectionTileSize': 1600
      ...
      this.osd.addHandler('full-screen', function(e) {
          ...
          console.log(e);
          if (!e.fullScreen) {
              //coming back from OSD's fullscreen drops the bottom status bar //force window to resize?
          }
      });
    ...
    renderChoices: function() {
      ...
      if (this.currentImg.choices.length > 0) {
        ...
        this.elemChoice.tooltipster({
          arrow: true,
          content: $.Templates.stitchView.imageChoices({ choicesInfo: choicesInfo }),
          functionReady: function() { _this.addImageChoiceEvents(); },
          interactive: true,
          position: 'bottom',
          theme: '.tooltipster-mirador'
    ...
    addToolbarNav: function() {
      this.parent.toolbar.append($.Templates.stitchView.navToolbar({
        navToolbarCls: this.navToolbarCls,
        hasAnnotations: this.currentImg.annotations
      }));
    ...
    addStatusbarNav: function() {
      this.parent.statusbar.append($.Templates.stitchView.statusbar({
        statusbarCls: this.statusbarCls,
        width: this.width,
        height: this.height
      }));
    ...
    // needs to be something else (combine two pages for paged object, manifest title for continuous)
    getTitle: function() {
      var titles = [];
    ...
    attachNavEvents: function() {
      var navToolbar = this.parent.toolbar.element.find('.' + this.navToolbarCls),
      selectorAnnotationsView = '.mirador-icon-annotations',//will this work?
      selectorMetadataView    = '.mirador-icon-metadata-view',
      selectorScrollView      = '.mirador-icon-scroll-view',
      selectorThumbnailsView  = '.mirador-icon-thumbnails-view',
      selectorNext            = '.mirador-icon-next',//next two pages for paged
      selectorPrevious        = '.mirador-icon-previous',//previous two pages for paged
      _this = this;
  ...
  $.ThumbnailsView.prototype = {
    ...
    attachNavEvents: function() {
      ...
      jQuery(selectorImageLinks).on('click', function(event) {
        ...
        $.viewer.loadView("stitchView", _this.manifestId, imageId); //testing
      });
