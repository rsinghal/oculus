...
(function($) {
  ...
  $.StitchView = function(options) {
    ...
    jQuery.extend(this, {
      currentImg:       null,
      currentImgIndex:  0,
      leftIndex:        [],//maybe doesn't need to be global
      rightIndex:       [],//maybe doesn't need to be global
      topIndex:         [],//maybe doesn't need to be global
      bottomIndex:      [],//maybe doesn't need to be global
      stitchList:       [],
      element:          null,
      elemChoice:       null,
      height:           null,
  ...
  $.StitchView.prototype = {
    ...
    render: function() {
      this.addToolbarNav();
      //this.createOpenSeadragonInstance(this.currentImg.imageUrl); 
      this.createOpenSeadragonInstance(this.stitchList); //send stitchList
      this.addStatusbarNav();
      this.attachWindowEvents();
      this.addAnnotationsLayer();
    },

    // need list of imageUrls
    createOpenSeadragonInstance: function(stitchList, osdBounds) {
      //need to get list of infoJsonUrls
      //var infoJsonUrl = $.Iiif.getUri(imageUrl) + '/info.json', 
      ...
      this.osd = $.OpenSeadragon({
        'id':           elemOsd.attr('id'),
        'toolbar':      osdToolBarId,
        'tileSources':  tileSources, //$.Iiif.prepJsonForOsd(infoJson) // need to return list
	'collectionMode':     'true',
	'collectionRows':     1,
