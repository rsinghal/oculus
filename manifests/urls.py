...
urlpatterns = patterns('',
    url(r'^(?P<view_type>view(-dev|-annotator|-m1)?)/(?P<document_id>([a-z]+:[A-Za-z\d]+;?)+)$', views.view, name='view'),
...
#    url(r'^(?P<view_type>view(-dev|-annotator|-m2)?)/images/openseadragon/(?P<filename>.*)$', views.get_image), 
   url(r'^(?P<view_type>view(-dev|-annotator|-m1)?)/images/(?P<filename>.*)$', views.get_image), 
    url(r'^(?P<view_type>view(-dev|-annotator|-m1)?)/+.*skins.*$', views.clean_url), 
)
